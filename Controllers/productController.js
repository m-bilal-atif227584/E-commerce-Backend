...
export const fetchAllProducts = catchAsyncErrors(async (req, res, next) => {
  ...
  //http://localhost:4000/products?category=smartphones => Its a query 
  //http://localhost:4000/product/12455465 => Number after product/ is product's id which is called params.
 
  const page = parseInt(req.query.page) || 1;
  const limit = 10;
  const offset = (page - 1) * limit;
  //Its logic to divide all products among multiple pages. jese k page 1 pr 10 products show honge phir second page pr products 11 se 20 tak fetch honge.
  ...
  const whereClause = conditions.length
    ...
    : "";  //it will add AND between conditions values of array agar conditions ki length hai to aur ye AND is liye laga rahe because hum iski query chalaenge aur postgres query me AND aata hai conditions k beech me. 
  ...
  const topRatedQuery = `
    ...
    LIMIT 8
  `; // We can fetch data from multiple tables using LEFT JOIN. Here we are fetching reviews whose r.product_id is equal to p.id. (p.id from products table).
  ...
  res.status(200).json({
    ...
    topRatedProducts: topRatedResult,
  });
});
